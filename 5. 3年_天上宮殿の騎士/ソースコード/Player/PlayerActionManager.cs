...
public class PlayerActionManager : SingletonMonoBehaviour<PlayerActionManager>
{
    ...
    [SerializeField, Tooltip("追従させるParentConstraint")]
    ...
    private void Start()
    {
        ...
        _broker.Receive<PlayerEvent.OnParentChangeToObject>()
            /*.Where(_ => PlayerStateManager.HasFlag(PlayerStatus.PlayerState.HangingL) || 
                        PlayerStateManager.HasFlag(PlayerStatus.PlayerState.HangingR))*/
            .Subscribe(x =>
            {
                ParentChange(x.HitObject, _parentConstraint);
            }).AddTo(this);
    }
